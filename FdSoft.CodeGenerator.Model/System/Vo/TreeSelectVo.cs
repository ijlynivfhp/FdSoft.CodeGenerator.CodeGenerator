...
namespace FdSoft.CodeGenerator.Model.System.Vo
{
    /// <summary>
    /// Treeselect树结构实体类
    /// </summary>
    public class TreeSelectVo
    {
        ...
        public TreeSelectVo(MaterialGoodsType GoodsType)
        {
            Id = GoodsType.MaterialTypeId;
            Label = GoodsType.TypeName;

            //menu.getChildren().stream().map(TreeSelect::new).collect(Collectors.toList()); java写法
            List<TreeSelectVo> child = new List<TreeSelectVo>();
            foreach (var item in GoodsType.Children)
            {
                child.Add(new TreeSelectVo(item));
            }
