...
namespace FdSoft.CodeGenerator.Repository
{
    ...
    public class BaseRepository<T> : SimpleClient<T> where T : class, new()
    {
        ...
        public int AddAndReturnId(T parm, bool ignoreNull = true)
        {
            return Context.Insertable(parm).IgnoreColumns(ignoreNullColumn: ignoreNull).ExecuteReturnIdentity();
        }

        public int InsertAndReturnId(T parm, Expression<Func<T, object>> iClumns = null, bool ignoreNull = true)
        {
            return Context.Insertable(parm).InsertColumns(iClumns).IgnoreColumns(ignoreNullColumn: ignoreNull).ExecuteReturnIdentity();
        }
