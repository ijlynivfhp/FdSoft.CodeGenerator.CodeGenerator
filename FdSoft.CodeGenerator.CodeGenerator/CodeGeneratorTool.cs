...
using System.IO;
using System.Linq;
using FdSoft.CodeGenerator.CodeGenerator.Model;
using FdSoft.Common;
using FdSoft.CodeGenerator.Model.System.Generate;
...
namespace FdSoft.CodeGenerator.CodeGenerator
{
    /// <summary>
    /// 代码生成器
    /// </remarks>
    ...
    public class CodeGeneratorTool
    {
        ...
        public static void FdGenerate(GenerateDto dto)
        {
            ...
            dto.VueParentPath = dto.VueVersion == 3 ? "FdSoft.CodeGenerator-vue" : "FdSoft.CodeGenerator.Vue";
            if (!vuePath.IsEmpty())
            {
        ...
        private static void GenerateService(ReplaceDto replaceDto, GenerateDto generateDto)
        {
            ...
            var fullPath2 = Path.Combine(_option.IServicsNamespace, _option.SubNamespace, $"IService", $"I{replaceDto.ModelTypeName}Service.cs");
        ...
        /// <returns></returns>
        public static GenTable InitTable(string dbName, string userName, string tableName, string desc, string moduleName = default)
        {
            GenTable genTable = new()
            {
                DbName = dbName,
                BaseNameSpace = "FdSoft.CodeGenerator.",//导入默认命名空间前缀
                ModuleName = moduleName ?? "business",//导入默认模块名
        ...
        public static string GenTablesByDb(string tables, string moduleName = default, string dbName = "LaborSys")
        {
            if (string.IsNullOrEmpty(tables)) return "表不能为空";
            string[] tableNames = tables.Split(',', StringSplitOptions.RemoveEmptyEntries);
            CodeGeneraterService codeGeneraterService = InternalApp.ServiceProvider.GetService<CodeGeneraterService>();
            IGenTableColumnService genTableColumnService = InternalApp.ServiceProvider.GetService<GenTableColumnService>();
            IGenTableService genTableService = InternalApp.ServiceProvider.GetService<GenTableService>();
            foreach (var tableName in tableNames)
            {
                var tabInfo = codeGeneraterService.GetTableInfo(dbName, tableName);
                if (tabInfo != null)
                {
                    GenTable genTable = CodeGeneratorTool.InitTable(dbName, "Console", tableName, tabInfo?.Description, moduleName ?? "Labor");
                    genTable.TableId = genTableService.ImportGenTable(genTable);

                    if (genTable.TableId > 0)
                    {
                        //保存列信息
                        List<DbColumnInfo> dbColumnInfos = codeGeneraterService.GetColumnInfo(dbName, tableName);
                        ...
                        var genTableInfo = genTableService.GetGenTableInfo(genTable.TableId.ParseToLong());
                        genTableInfo.Columns = genTableColumnService.GenTableColumns(genTable.TableId.ParseToLong());
                        string tempPath = AppDomain.CurrentDomain.BaseDirectory;
                        var parentPath = Directory.GetParent(tempPath)?.Parent?.Parent?.Parent?.Parent?.FullName;

                        //配置生成的路径
                        genTableInfo.GenPath = parentPath;
                        //生成代码到指定文件夹
                        CodeGeneratorTool.FdGenerate(new GenerateDto()
                        {
                            TableId = genTable.TableId,
                            DbType = 1,
                            GenTable = genTableInfo,
                            GenCodePath = genTableInfo.GenPath.IsEmpty() ? parentPath : genTableInfo.GenPath
                        });
                    }
                }
            }
