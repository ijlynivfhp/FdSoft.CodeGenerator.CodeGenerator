...
namespace FdSoft.CodeGenerator.Core.Extensions
{
    public static class SwaggerExtension
    {
        ...
        public static void UseSwagger(this IApplicationBuilder app,string swaggerName)
        {
            app.UseSwagger(c =>
            {
            ...
            app.UseSwaggerUI(c => c.SwaggerEndpoint("v1/swagger.json", $"FdSoft.CodeGenerator.{swaggerName} v1"));
        }

        public static void AddSwaggerConfig(this IServiceCollection services,string swaggerName)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                ...
                {
                    Title = $"FdSoft.CodeGenerator.{swaggerName}.NET Api - .NET6",
                    Version = "v1",
                    Description = "",
                });
                try
                {
                    //添加文档注释
                    c.IncludeXmlComments($"FdSoft.CodeGenerator.{swaggerName}.xml", true);
                }
                catch (Exception ex)
                {
