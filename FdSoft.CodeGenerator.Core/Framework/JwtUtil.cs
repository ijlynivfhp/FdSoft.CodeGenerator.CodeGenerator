...
namespace FdSoft.CodeGenerator.Core.Framework
{
    /// <summary>
    /// 2020-11-20
    /// </summary>
    public class JwtUtil
    {
        ...
        /// <param name="httpContext"></param>
        /// <returns></returns>
        public static LoginBaseDto GetLoginUserNew(HttpContext httpContext)
        {
            string token = httpContext.GetToken();
        ...
        public static TokenValidationParameters ValidParameters()
        {
            ...
            {
                throw new CustomException(ResultCode.CUSTOM_ERROR, "JwtSettings获取失败");
            }
            var key = Encoding.ASCII.GetBytes(jwtSettings.SecretKey);
        ...
        public static LoginUser? ValidateJwtToken(IEnumerable<Claim> jwtToken)
        {
            try
            {
                ...
                var loginUser = JsonConvert.DeserializeObject<LoginUser>(userData??string.Empty);
                return loginUser;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="jwtToken"></param>
        /// <returns></returns>
        public static LoginBaseDto? ValidateJwtTokenNew(IEnumerable<Claim> jwtToken)
        {
            try
            {
                var userData = jwtToken.FirstOrDefault(x => x.Type == ClaimTypes.UserData)?.Value ?? string.Empty;
                var loginBaseDto = JsonConvert.DeserializeObject<LoginBaseDto?>(userData);
                return loginBaseDto;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="user"></param>
        /// <returns></returns>
        public static List<Claim> AddClaimsNew(LoginBaseDto user)
        {
            var claims = new List<Claim>()
                {
                    new Claim(ClaimTypes.PrimarySid, user.UserId.ToString()),
