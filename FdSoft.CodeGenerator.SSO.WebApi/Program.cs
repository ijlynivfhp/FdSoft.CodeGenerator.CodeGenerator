...
using FdSoft.CodeGenerator.Core.Middleware;
using FdSoft.CodeGenerator.SSO.WebApi.Hubs;
...
Task.Run(() =>
{
    ...
    RedisHelper.Initialization(new CSRedis.CSRedisClient(AppSettings.GetConfig("RedisServer:Default")));
});
...
builder.Services.AddSwaggerConfig(GlobalConstant.SSO);
...
app.UseSwagger(GlobalConstant.SSO);
